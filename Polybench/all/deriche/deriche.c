		 ...
		 DATA_TYPE POLYBENCH_2D(imgOut,W,H,w,h))
{
  ...
  *alpha=0.25; //parameter of the filter
  ...
  for (i = 0; i < w; i++)
     for (j = 0; j < h; j++)
	imgIn[i][j] = (DATA_TYPE) ((313*i+991*j)%65536) / 65535;
}
...
/* Start of CLooG code */
k = (SCALAR_VAL(1)-EXP_FUN(-alpha))*(SCALAR_VAL(1)-EXP_FUN(-alpha))/(SCALAR_VAL(1)+SCALAR_VAL(2)*alpha*EXP_FUN(-alpha)-EXP_FUN(SCALAR_VAL(2)*alpha));;
a1 = a5 = k;;
a2 = a6 = k*EXP_FUN(-alpha)*(alpha-SCALAR_VAL(1));;
a3 = a7 = k*EXP_FUN(-alpha)*(alpha+SCALAR_VAL(1));;
a4 = a8 = -k*EXP_FUN(SCALAR_VAL(-2)*alpha);;
b1 = POW_FUN(SCALAR_VAL(2),-alpha);;
b2 = -EXP_FUN(SCALAR_VAL(-2)*alpha);;
c1 = c2 = 1;;
if (_PB_H >= 1) {
